...
module.exports =
  props: <[ columns rows total actions endpoint prefix query expand toggle update insert  ]>
  created: ->>
    @rowscache = @rows
  ...
  methods:
    ...
    refresh: ->>
      res = await @$store.dispatch "#{@endpoint}/find"
      @rowscache = res.data
    ...
    toggleStatus: (intent) ->>
      ...
      try 
        ...
        payload = [ id, { urlSuffix: 'status', status } ]
        console.log "#{@endpoint}/patch" payload
        await @$store.dispatch "#{@endpoint}/patch" payload
