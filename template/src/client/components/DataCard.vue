...
module.exports = 
  props: <[ model schema validation endpoint mode title ]>
  data: ->
    immutable: true
    submitting: false
    form:
  ...
  computed:
    ...
    titre: -> if @title then @title else "#{@intent} #{@entity}"
    entity: -> 
      if @endpoint
        str = @endpoint
    ...
    upsert: ->>
      @$refs['form'].validate (valid) ~>>
        ...
        if not valid
          ...
          try
            payload = if @mode is 'write' then [ data, params ]  else [ data.id, data, params ]
            res = await @$store.dispatch "#{@endpoint}/#{path}" payload
            #res = await @$store.app.api.service(@endpoint)[path] payload
            console.log "upsert@DataCard #{@intent} :: #{@endpoint}/#{path}", payload, #res 
            @$Message.info content: "Update Successful", duration: 16 
            @cancel!
          catch
  ...
  created: ->>
    ...
    @formModel = @model
    console.log 'DataCard created' @mode, @endpoint, @model
  components:
