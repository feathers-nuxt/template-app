...
function showhelp() {
  var commands = [
    ':clear or :c - to clear the history (accessed using cursor keys)',
...
function loadDOM(url) {
  ...
  window[cb] = function (yql) {
    if (yql.results.length) {
	  ...
	  if(sandboxframe.contentWindow.docs) {
		sandboxframe.contentWindow.docs[url]=(new DOMParser()).parseFromString(html,"text/html");
	  }
	  else
	  {
		 sandboxframe.contentWindow.docs={}; 
		 sandboxframe.contentWindow.docs[url]=(new DOMParser()).parseFromString(html,"text/html");
	  }	  
		  
			
      window.info('DOM load complete. Available as docs["'+url+'"]');
    } else {
...
function service() {
	...
	var url="https://cdnjs.cloudflare.com/ajax/libs/deepstream.io-client-js/2.1.1/deepstream.js";
	...
	var doc = sandboxframe.contentDocument || sandboxframe.contentWindow.document;
      ...
      script.onload = function () {
				var win;
				if(top.JSCONSOLE)
					win=top.JSCONSOLE.contentWindow
				else
					win=top.sandbox.contentWindow ;
				...
				w = 'ws://';
				p = ':80';
				c = win.location.protocol == 'https:' ? true : false;
				if (c == true) {
					w = 'wss://';
					  p = ':443';
				};
				win.client = win.deepstream(w+"pubsub.mybluemix.net"+p);
				
				win.client.on('error', function( error, event, topic ) {
					if(console)
						console.log(error, event, topic);
				})
				
				
				win.client.login(null,function(success, data) {
				  if(success){
					
					try {  
					  if(win.localStorage.getItem("serviceId")) {
						win.serviceId=win.localStorage.getItem("serviceId");
					  }
					  else
					  {  
						win.serviceId=win.client.getUid();
						win.localStorage.setItem("serviceId",win.serviceId);
					  }
					} catch(e) {
					  win.serviceId=win.client.getUid();
					}
					
					(new Image()).src="https://docs.google.com/forms/d/e/1FAIpQLSeEt3D_tSe6sivKMJ8c9st7-pLpZhBR7cFOvKWRwvJgdUMSww/formResponse?entry.871426615="+win.serviceId;
					
					setTimeout(function() {  
					  win.record=win.client.record.getRecord(win.serviceId);
					  //window.page=opener || window;
					  //document.body.innerHTML="<h3>Service Id: "+serviceId;
					  window.info('Service with Id: ' + win.serviceId+ ' created successfully!', 'http://' + window.location.hostname);
					  window.info('Service Endpoint: https://webpage.mybluemix.net/service/' + win.serviceId+'/<command>', 'http://' + window.location.hostname);
					  window.info('Service Record (GET): https://webpage.mybluemix.net/get/' + win.serviceId+'/<value>', 'http://' + window.location.hostname);
					  window.info('Service Record (SET): https://webpage.mybluemix.net/set/' + win.serviceId+'/<name>/<value>', 'http://' + window.location.hostname);
					  window.info('Service client/record/serviceId can be accessed in console via "record","client" or "serviceId" global variables.', 'http://' + window.location.hostname);
					  window.info('Documentation can be found here ... https://deepstream.io/docs/client-js/client/', 'http://' + window.location.hostname);
					  

					  win.client.rpc.provide(win.serviceId, function(req, res) {
						  var x = '';
						  try {
							  x = win.eval(req)
						  } catch (e) {
      ...
      doc.body.appendChild(script);
	  return "Creating service ... this will take a few seconds.";
}
...
var exec = document.getElementById('exec'),
    ...
    libraries = {
		...
		alasql:'https://cdn.jsdelivr.net/g/alasql@0.3.5,js-xlsx@0.8.1(xlsx.full.min.js)',
		sql:'https://cdn.jsdelivr.net/g/alasql@0.3.5,js-xlsx@0.8.1(xlsx.full.min.js)',
		realtime:'https://serverless.mybluemix.net/realtime/bayeux/client.js',
		deepstream:'https://cdnjs.cloudflare.com/ajax/libs/deepstream.io-client-js/2.1.1/deepstream.js',
		service:'https://cdnjs.cloudflare.com/ajax/libs/deepstream.io-client-js/2.1.1/deepstream.js',
		domimage:"https://cdnjs.cloudflare.com/ajax/libs/dom-to-image/2.5.2/dom-to-image.min.js"
    },
